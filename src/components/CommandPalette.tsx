...
export function CommandPalette({ open, actions, onClose }: { open: boolean; actions: CommandAction[]; onClose: () => void }) {
  ...
  const filtered = useMemo(() => {
    ...
    return actions.filter(action => `${action.label} ${action.group} ${action.keywords ?? ''}`.toLowerCase().includes(needle)).slice(0, 40);
  }, [actions, query]);
