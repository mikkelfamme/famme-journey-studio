...
function SaveIndicator({ state, lastSavedAt, error }: { state: SaveState; lastSavedAt?: string; error?: string }) {
  if (state === 'saving') return <div className="save-indicator saving" title="Saving changes locally"><Loader2 size={13}/><span>Saving…</span></div>;
  if (state === 'error') return <div className="save-indicator error" title={error || 'Autosave failed'}><AlertCircle size={13}/><span>Save error</span></div>;
  if (state === 'saved') return <div className="save-indicator saved" title={lastSavedAt ? `Last saved ${new Date(lastSavedAt).toLocaleTimeString()}` : 'Saved locally'}><CheckCircle2 size={13}/><span>Saved</span></div>;
  return <div className="save-indicator"><span>Local</span></div>;
}
  ...
  return (
    <header className="topbar">
      ...
      <div className="topbar-actions">
        {canInstall && <button className="button install-button" onClick={()=>void install()} title="Install Famme Journey Studio as an app"><MonitorDown size={16}/> Install app</button>}
        ...
        {onNew && <button className="button primary" onClick={onNew}><Plus size={16} /> New journey</button>}
        <button className="button" onClick={onImport}><FileUp size={16} /> Import data</button>
        <button className="button" onClick={onExport}><Download size={16} /> Export .fjs</button>
      </div>
    </header>
  );
}
