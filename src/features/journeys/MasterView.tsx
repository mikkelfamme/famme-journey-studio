...
export function MasterView() {
  ...
  const selectedJourney = detail ? workspace.journeys.find(j => j.id === detail.journeyId) : undefined;
  ...
  function DetailBody() {
    ...
    if (detail.kind === 'handoff') {
      ...
      return <div className="master-detail-list">{links.length === 0 ? <div className="empty-mini">{isDa ? 'Ingen handoffs. Kunderejsen har ingen eksplicitte forbindelser til andre kunderejser.' : 'No handoffs. This journey has no explicit links to another journey.'}</div> : links.map(link => {
        ...
        const targetJourney = workspace.journeys.find(j => j.id === link.targetJourneyId);
    ...
    const coverage = coverageForJourney(workspace, selectedJourney.id, snapshot);
    const nodeRows = selectedJourney.nodes.map(node => {
      ...
      const qualities = records.map(record => mappingQuality(workspace, record));
  ...
  return <section className="content-section master-page">
    ...
    {workspace.journeys.length===0?<div className="empty-state"><GitBranch size={32}/><h3>{t('master.empty')}</h3><p>{t('master.emptyText')}</p></div>:<div className="master-list">{workspace.journeys.map(j => { const issues=validateJourney(j); const todos=j.nodes.reduce((sum,n)=>sum+n.data.annotations.filter(a=>a.kind==='todo'&&!a.done).length,0); const coverage=coverageForJourney(workspace,j.id,snapshot); return <div className="master-row" key={j.id}><div><strong>{j.name}</strong><span>{j.scope} · {status(j.status)}</span></div><div className="master-path">{[...j.nodes].sort((a,b)=>a.position.x-b.position.x).slice(0,6).map((n,i)=><span key={n.id}>{i>0 && <em>→</em>}{n.data.label}</span>)}</div><div className="master-badges"><button className={`chip master-badge-button ${issues.length ? 'chip-warn':''}`} onClick={()=>setDetail({journeyId:j.id,kind:'health'})}>{issues.length} health</button><button className="chip master-badge-button" onClick={()=>setDetail({journeyId:j.id,kind:'todo'})}>{todos} todo</button><button className="chip master-badge-button" onClick={()=>setDetail({journeyId:j.id,kind:'handoff'})}>{j.crossJourneyLinks.length} handoff</button><button className="chip master-badge-button" onClick={()=>setDetail({journeyId:j.id,kind:'mapped'})}>{coverage.pct}% mapped</button></div></div>})}</div>}
