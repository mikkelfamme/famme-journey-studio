...
export function ComponentsView() {
  const { workspace, updateWorkspace } = useWorkspace(); if (!workspace) return null;
  function usage(componentId: string) { return workspace.journeys.reduce((sum, journey) => sum + journey.nodes.filter(node => node.data.componentId === componentId).length, 0); }
  function syncAll(componentId: string) {
    const component = workspace.components.find(c => c.id === componentId); if (!component) return;
