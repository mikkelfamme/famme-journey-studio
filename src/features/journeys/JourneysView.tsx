...
export function JourneysView({ onEdit, onView }: { onEdit: (journey: Journey) => void; onView: (journey: Journey) => void }) {
  ...
  function create() {
    const template = currentWorkspace.templates.find(item => item.id === templateId) ?? currentWorkspace.templates[0];
    if (!template) return;
    const name = window.prompt(t('journeys.namePrompt'), `New ${template.name}`);
    if (!name) return;
